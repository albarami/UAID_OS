...
#### Slice 53 — Production-approval workflow → verified A5 pre-approval (gate #12) — **MERGED (PR #96, `5fd8b18`)**
- **Goal.** Record a narrowly truthful request-authenticated production pre-approval under fixed, recorded conditions, bound to one exact current frozen candidate, re-audited evidence core, gate-eligible verdict, autonomy policy, and canonical policy/checklist; make gate #12 PASS-capable without inventing a human-signature tier or production authority.
- **Why now.** Gate #12 was the last pre-control-loop approval gate and its Slice-27 identity and Slice-33 channel prerequisites already existed; the implementation removes only the named unimplemented-subsystem blocker while retaining the literal hard-false until Slice 55.
- **Spec grounding.** §18.2 (1769); §24.1; App. B #12 (2996); tmpl `20_*`, `23_go_live_checklist.yaml`; §5.2 "Deploy production — A4/A5".
- **Files.** `app/release/production_approval.py`, `app/release/production_approval_service.py`, `app/models/production_preapproval.py`, `app/repositories/production_preapprovals.py`, `app/api/production_preapprovals.py`, the approval/channel extensions, and the gate-#12 ladder in `app/release/production_autonomy.py`.
- **Migration.** `0052_production_preapprovals` — five additive tenant-owned, RLS ENABLE+FORCE, append-only policy/request/attestation/lifecycle tables and two additive composite identity targets on intake categories and autonomy policies.
- **Tenant/RLS/FK/audit/immutability.** Same-tenant/project/candidate/core/verdict/policy bindings; immutable requests and attestations plus append-only linear lifecycle; deferred DB guards re-derive policy membership, requester/approver separation, complete graph shape, digests, currentness, and eligibility; only principal digests persist and audit is safe metadata only.
- **Tests.** Strict template projection; bodyless mutation API/OpenAPI contract; ≤24-hour expiry boundaries; latest-negative supersession; direct-SQL self-approval/forged-graph rejection; cross-tenant/RLS/audit sentinels; all-gate golden matrix including a synthetic all-thirteen-pass report. Verified suites: 1060 Docker-free / 819 DB-backed.
- **A5 gate(s) advanced.** **#12 → PASS-capable** under `slice53.v1`, the twelfth pass-capable gate; the second no-go reason is removed because the subsystem exists, while the literal `False` and `a5_gates_not_all_satisfied` remain.
- **Must NOT claim.** That request authentication is a human signature or human-presence proof, that caller-supplied policy is verified organizational authority, that a passing gate #12 satisfies A5, or that pre-approval authorizes production.
- **Exit.** SATISFIED by PR #96 (`5fd8b18`): gate #12 can pass from a current request-authenticated pre-approval under recorded conditions; readiness stays `slice20.v1`, gate #13 remains sourceless, and go-live remains literal hard-false.

#### Slice 54 — Emergency stop / rollback authority (gate #13) — **MERGED (PR #98, `84e955a`)**
- **Goal.** Provide a real project-scoped DB latch enforced over the current UAID runtime and an exact release-bound rollback-authority record, making gate #13 PASS-capable without claiming production incident execution.
- **Why now.** Gate #13 was the final Appendix-B gate without a pass path and is required before Slice 55 can design the governed §23.3 control loop. The implementation reuses the Slice-53 recorded-policy/key-custody boundary and the Slice-52 connector-observed staging rollback evidence.
- **Spec grounding.** App. B #13 (2997); §25.2 (2381–2389); §18.2 emergency-rollback row; App. C l.3016 says only that production overrides require explicit authority — the local executing latch is the ruled conservative implementation, not a verbatim Appendix-C mechanism mandate.
- **Files.** `app/release/emergency_stop.py`, `app/release/emergency_control_service.py`, `app/models/emergency_control.py`, `app/repositories/emergency_controls.py`, `app/api/emergency_controls.py`, the runtime/repository latch checks, and the gate-#13 ladder in `app/release/production_autonomy.py`.
- **Migration.** `0053_emergency_controls` — five additive tenant-owned, RLS ENABLE+FORCE, append-only emergency-control tables; `run_steps` gains `emergency_paused`; `project_runs` gains the active-latch guard; downgrade restores the exact pre-0053 runtime shape.
- **Tenant/RLS/FK/audit/immutability.** Standing project latch plus current candidate/core/Slice-52-run rollback binding; exact Slice-53 policy-member equality; append-only linear event history; two-person clear; activation effects and no-running-while-active enforced at commit; role-code-only safe audit metadata.
- **Tests.** All eight runtime entry points and node-boundary checks; checkpointed node-B refusal after activation; activation/restart race; direct-SQL graph/effect/running-state forgery refusal; unauthorized and same-actor clear refusal; clear-never-resumes; active-stop gate block; audit sentinel; golden A5 and hard-false matrices. Verified suites: 1083 Docker-free / 826 DB-backed.
- **A5 gate(s) advanced.** **#13 → PASS-capable** under `slice54.v1`, the thirteenth and final PASS-capable gate; gates #1–#12 remain regression-proven unchanged.
...
- **Exit.** SATISFIED by PR #100 (`15d0e75`): independent review APPROVE, CI green, and the bounded non-executing control loop merged. Salim personally reviewed PR #100 and RELEASED Slice 56 (2026-08-22).
...
- **Files.** `app/ops/{signals,assessment,collect,db_checks}.py`, `app/models/ops_signal.py`, `app/repositories/ops_signals.py`, migration `0055_ops_signals`.
- **Migration.** `0055` (`down_revision=0054`) — `ops_observation_runs` + `ops_signal_results`; tenant-owned, RLS ENABLE+FORCE, append-only; dual deferred count-match; money CHECKs refuse NaN/±Infinity.
- **Tenant/RLS/FK/audit/immutability.** Same-tenant/project composite bindings; SELECT/INSERT only; audit counts/ids/digests only. Public collect owns REPEATABLE READ; no caller `session`/`as_of`.
- **Tests.** Default counters `2/0/9`; sample overlay + refusals; cost no-budget `not_evaluable`; RLS/append-only/count-match/window-end/idempotency race; empty Alembic round-trip; scoped pyright 0. Verified suites: `make test` 1160 / 861 deselected; `make test-db` 861 / 1160 deselected.
- **A5 gate(s) advanced.** None. A5 stays `slice54.v1`; readiness `slice20.v1`; go-live stays literal `False`.
...
- **Files.** `app/ops/{incidents,incident_service,incident_ddl,incident_db_checks}.py`, `app/models/ops_incident.py`, `app/repositories/ops_incidents.py`, `app/repositories/ops_incident_reads.py`, migration `0056_ops_incidents`.
- **Migration.** `0056` (`down_revision=0055`) — six tenant-owned RLS ENABLE+FORCE tables + additive UNIQUE targets on `ops_signal_results` and `pm_issue_mappings`. Incidents SELECT/INSERT/UPDATE no DELETE; other five SELECT/INSERT append-only.
- **Tenant/RLS/FK/audit/immutability.** Same-tenant/project composite bindings; seq-1 ticket insert-before-results; dual deferred count-match of seven children; audit ids/status/counts/decisions only.
- **Tests.** Policy-gated tickets via real `decision_for`; seq 2 unevaluated; seq 6–7 never `local_ticket_written`; no `accepted`; handover-only populated downgrade refusal; self-seeding catalog/RLS/append-only; empty Alembic round-trip; scoped pyright 0. Verified suites: `make test` 1172 / 873 deselected; `make test-db` 873 / 1172 deselected.
- **A5 gate(s) advanced.** None. A5 stays `slice54.v1`; readiness `slice20.v1`; go-live stays literal `False`.
- **Must NOT claim.** Live IR; Jira writes; log diagnosis; hotfix/deploy/rollback execution; Slice 56 `incident_reports` equals this workflow; production hotfix without approval.
- **Exit.** Incident ledger + authorized local tickets + handover recorded. Self-healing remains Slice 58.

#### Slice 58 — Self-healing / hotfix + rollback paths — **MERGED AS NON-CLOSING (PR #106, `787ddd6`)**
- **Goal (as shipped).** Record a tenant-owned **hotfix-intent evaluation** for an existing non-terminal incident: local A2 branch/PR plans under one locked policy snapshot, fail-closed production non-execution, and **current** Slice 52/54 rollback *context* when the complete gate-#10 conjunction holds. This does **not** close spec §26.6 or satisfy the original roadmap Slice 58 exit.
- **Why now.** Needed after the incident workflow (S57) plus rollback (S52) + emergency authority (S54): remediation intent must remain autonomy/policy-governed (§25.2).
- **Spec grounding.** §25.2 (2381–2389); §26.6 "self-healing/hotfix loop"; `.planning/SLICE-58-PLAN.md` v3 (OD-58-1…10 = Option A).
- **Files.** `app/ops/{hotfix,hotfix_service,hotfix_ddl,hotfix_db_checks}.py`, `app/models/ops_hotfix.py`, `app/repositories/ops_hotfix.py`, additive `snapshot_decisions` / `coverage_with_run` / authorization match helpers, migration `0057_self_healing`.
- **Migration.** `0057` (`down_revision=0056`) — three tenant-owned RLS ENABLE+FORCE append-only tables + additive `uq_era_id_project_tenant`. SELECT/INSERT only.
- **Tenant/RLS/FK/audit/immutability.** Same-tenant/project composite bindings; latch `assert_project_not_stopped` before INSERT; child `policy_decision` must match `decision_snapshot`; seq-4 `plan_written` requires a same-run `patch_branch` plan; audit ids/status/counts/decisions only.
- **Tests.** Policy-first seq 5–7; A2 ALLOW writes branch+PR; DENY/NEEDS_APPROVAL write zero plans; branch-before-PR DB guard; exact gate-10 run from one `coverage_with_run`; latch/stale/old-authorization refusals; no git/broker; scoped pyright 0. Verified suites: `make test` 1189 / 888 deselected; `make test-db` 888 / 1189 deselected.
- **A5 gate(s) advanced.** None. A5 stays `slice54.v1`; readiness `slice20.v1`; go-live stays literal `False`.
...
- **Migration.** Number from Alembic head at Slice-60 plan time (`0057` is occupied by Slice 58). Additive export/signature metadata if needed.
...
- **Migration.** Number from Alembic head at Slice-62 plan time. Additive.
...
- **Files.** `app/admin/*`.
- **Migration.** Number from Alembic head at Slice-63 plan time. Additive admin tables.
...
- **Exit.** **HALTED (plan line).** Three consecutive Sol plan REJECTs (v1, v2, v3). No product code. Alembic head still `0061`. No v4 without owner. D-8/D-9/D-10 and the Slice 61 exit stay OPEN.
...
**HALTED: Slice 63 plan line.** Sol REJECT of v3 is consecutive plan REJECT #3. Defect: writer-function mutations in `.planning/SLICE-63-PLAN.md` §5.2.a remain masked by `admin_policy_changes_guard`; live PostgreSQL 16 probes commit only when that neighbouring trigger is also disabled, so `P-writer-no-existing-policy` does not close v2 defect 3. No v4, no implementation, no product PR until the owner authorizes. Alembic head remains `0061`. Slice 61 exit and D-8/D-9/D-10 stay OPEN. A5 `slice54.v1`, readiness `slice20.v1`, `can_go_live_autonomously=False`. Seat ruling (2026-08-23, standing) unchanged. After any future Slice 63 merge: stop and write the Slice 55–63 final report; do not start new work.
...
| support tickets, incident reports | assessed as `not_observed` in S56; create/triage/handover in S57 (§25.1/§25.2) | S56 (assessment) + S57 (workflow) |
| post-launch ticket creation | autonomous bug ticket (§25.2) | S57 |
| support handover | handover record (§25.4) | S57 |
| hotfix + rollback paths | self-healing/hotfix (§25.2) + rollback (S52) + emergency authority (S54) | S58 **intent evaluation only** (non-closing; residual actuators open) |
...
**Definition of "functional after go-live" (this roadmap's bar):** all eleven §25.1 classes *assessable* with honest `not_observed` where no live source exists (S56 — this is not adequate monitoring or proof production is live); incidents created/triaged/handed over (S57); governed self-healing/hotfix/rollback under autonomy policy (S58 — **intent evaluation merged; actuators still open**); stabilization window with enforced exit criteria, validated backup/restore, and authorized closure, feeding a continuous-improvement loop (S59). Until residual S58 actuators and S59 exist, the system can *pass* A5 gates but cannot *operate* post-launch as specified.
...
- **No implementation hidden in planning.** This Rev-20 follow-up records Slice 60 as a signed offline auditor bundle that **closes no spec section**, under the standing cadence through 63. `.env` remains ignored and unstaged.
- **No go-live overclaim.** `can_go_live_autonomously`/`a5_satisfied` remain false; Slice 55 records only `decided_not_executed`; Slice 58 does not create git branches, open PRs, deploy, or roll back production, and does not close §26.6. Go-live remains unauthorized. Slice 60 signs a detached manifest with an app-custody Ed25519 key; that is byte-identity evidence, not a human signature, authority attestation, PKI rooting, or authorization, and its declared expiry is not enforced.
