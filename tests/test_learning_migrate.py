"""Slice 62 Alembic 0061 reversibility (empty roundtrip + populated refusal)."""
...
from tests.learning_support import LEARNING_TABLES
...
async def _seed_run_row(database: str) -> None:
    conn = await _admin_connect(database)
    try:
        await conn.execute(
            "ALTER TABLE public.cross_project_aggregate_runs DISABLE TRIGGER ALL"
        )
        await conn.execute(
            "INSERT INTO public.cross_project_aggregate_runs "
...
@pytest.mark.db
def test_p20_alembic_0061_empty_roundtrip_and_populated_refusal(_schema) -> None:
    ...
    try:
        with _alembic_url(alembic_url) as cfg:
            ...
            assert asyncio.run(_regclass(dbname, "catalog_assets")) == "catalog_assets"
            command.downgrade(cfg, "0060")
            for table in LEARNING_TABLES:
                assert asyncio.run(_regclass(dbname, table)) is None
            assert asyncio.run(_regclass(dbname, "catalog_assets")) == "catalog_assets"
            command.upgrade(cfg, "0061")
            asyncio.run(_seed_run_row(dbname))
            with pytest.raises(DBAPIError, match="cannot downgrade Slice 62"):
                command.downgrade(cfg, "0060")
    finally:
